...
-- Status fetched from local (inlined below)
local StatusCache = {
        ["3392487150"] = "Indev",
        ["122764594952227"] = "Working | Beta",
        ["82638711520338"] = "Indev",
        ["120331934510435"] = "Indev",
        ["UNIVERSAL"] = "Indev",
}
local IconCache = {}
local GameRegistry = {}
...
Title.RichText = true
Title.Text = '<font color="#ffffff">HACKSENSE</font><font color="#ff6a85">.GOV</font>'
...
AddGame("Universal", 3392487150, function()
        -- Indev
        local Notification = Instance.new("Message")
        Notification.Text = "[HackSense.gov] Universal is in development."
...
AddGame("Europhium HvH", 82638711520338, function()
        -- Indev
        local Notification = Instance.new("Message")
        Notification.Text = "[HackSense.gov] Europhium support is in development."
        Notification.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
        task.delay(3, function() Notification:Destroy() end)
end)
AddGame("Sultanisimus HvH", 120331934510435, function()
        -- Indev
        local Notification = Instance.new("Message")
        Notification.Text = "[HackSense.gov] Sultanisimus support is in development."
        Notification.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
        task.delay(3, function() Notification:Destroy() end)
end)
