...
-- Testing Resolver v3 — BodyYaw Reader
-- Reads the game's own BodyYaw NumberValue to determine the REAL body position.
-- The game stores the exact desync angle in Character.BodyYaw.Value.
-- We use the SAME formula as the game's own resolver (calculateRealBodyPartPosition)
-- to calculate where the real hitbox is, making this far more accurate than
-- guessing from visual CFrame changes.
--
-- Auto-detects AA mode: LEGIT / STATIC / JITTER / DEFENSIVE
-- For JITTER: uses histogram-based mode finding + safety yaw noise
-- Visually locks resolved targets in place (undo their visual desync)

if not getgenv().TestingResolver then
    getgenv().TestingResolver = {
        Enabled = false,
        PredictionTick = 0,       -- movement prediction ticks (velocity-based)
        ServerDelayTicks = 2,     -- server replication delay compensation ticks
        JitterSafetyYaw = 5,      -- extra yaw noise for jitter targets (degrees)
        VelocitySamples = {},     -- per-player velocity history
    }
end

-- Per-player resolver state (local scope, not in getgenv)
local _rs = {}

local function _rsGet(plr)
    if not _rs[plr] then
        _rs[plr] = {
            bodyYawSamples = {},
            detectedMode = "LEGIT",
...
-- Classify AA mode from BodyYaw sample statistics
local function _rsClassify(samples)
    if #samples < 10 then return "LEGIT", 0 end
    local sum = 0
    for _, v in ipairs(samples) do sum = sum + v end
    local avg = sum / #samples
    local totalDelta = 0
    for i = 2, #samples do
        totalDelta = totalDelta + math.abs(samples[i] - samples[i - 1])
    end
    local avgDelta = totalDelta / (#samples - 1)
    if math.abs(avg) < 3 and avgDelta < 3 then
        return "LEGIT", 1.0
    elseif avgDelta > 15 then
        return "JITTER", 0
    elseif avgDelta > 4 then
...
-- For JITTER: find the mode (most common bucket) of BodyYaw samples
-- Uses 5-degree histogram buckets. Returns center of most common bucket + confidence.
local function _rsJitterMode(samples)
    local bucketSize = 5
    local counts = {}
    for _, v in ipairs(samples) do
        local bucket = math.floor((v + 180) / bucketSize) * bucketSize - 180
        counts[bucket] = (counts[bucket] or 0) + 1
    end
    local bestBucket, bestCount = 0, 0
    ...
    return bestBucket + bucketSize / 2, bestCount / #samples
end

-- Calculate real body part position using the game's OWN formula:
--   realRoot = hrp.CFrame * CFrame.Angles(0, math.rad(bodyYaw), 0)
--   relativeCF = hrp.CFrame:ToObjectSpace(part.CFrame)
--   realPartCF = realRoot * relativeCF
-- overrideYaw: optional, use a specific yaw instead of the current BodyYaw value
function GetRealPartPosition(target, partName, overrideYaw)
    local char = target.Character
    if not char then return nil end
    local hrp = char:FindFirstChild("HumanoidRootPart")
    local bodyYawValue = char:FindFirstChild("BodyYaw")
    local part = char:FindFirstChild(partName)
    if not hrp or not bodyYawValue or not part then
        return part and part.Position or nil
    end
    local yaw = overrideYaw or bodyYawValue.Value
    local yawRad = math.rad(yaw)
    local realRoot = hrp.CFrame * CFrame.Angles(0, yawRad, 0)
    local relativeCF = hrp.CFrame:ToObjectSpace(part.CFrame)
    return (realRoot * relativeCF).Position
end

-- Get resolved hitpos considering AA mode, jitter, and movement prediction
-- Returns the position to send as hitpos in the FireServer hook
function GetResolvedHitPos(target, partName)
    local s = _rsGet(target)
    local char = target.Character
    if not char then return nil end
    local hrp = char:FindFirstChild("HumanoidRootPart")
    local bodyYawValue = char:FindFirstChild("BodyYaw")
    ...
    if s.detectedMode == "LEGIT" then
        return part.Position
    end
    local yaw = bodyYawValue.Value
    if s.detectedMode == "JITTER" and s.confidence > 0.3 then
        -- Use locked (mode-based) yaw instead of current jittered value
        yaw = s.lockedYaw
        -- Add jitter safety noise (same technique the game uses)
        local safety = getgenv().TestingResolver.JitterSafetyYaw or 5
        local noise = (math.random() * 2 - 1) * safety
        yaw = yaw + noise
    end
    return GetRealPartPosition(target, partName, yaw)
end

-- Velocity tracking for movement prediction (kept from v2)
task.spawn(function()
    local Players = game:GetService("Players")
    local RunService = game:GetService("RunService")
    local LocalPlayer = Players.LocalPlayer
...
-- GetPredictedPosition: predict where a body part will be N ticks offset
-- Supports negative ticks (past) for server delay compensation
function GetPredictedPosition(target, partName, ticks)
    local samples = getgenv().TestingResolver.VelocitySamples
    local hrp = target.Character and target.Character:FindFirstChild("HumanoidRootPart")
    if not hrp then return nil end
    local entry = samples[target]
    local bodyOffset = Vector3.zero
    if partName then
        local part = target.Character:FindFirstChild(partName)
        if part then
            bodyOffset = part.Position - hrp.Position
        end
    end
    if not entry then
        return hrp.Position + bodyOffset
    end
    local tickDuration = 0.05
    local netOffset = (ticks or 0) * tickDuration
    local predictedHRP = hrp.Position + entry.velocity * netOffset
    return predictedHRP + bodyOffset
end

-- Resolver v3: update loop (AA detection, visual lock, death reset)
-- Runs every frame when Testing mode is active
task.spawn(function()
    local Players = game:GetService("Players")
    local RunService = game:GetService("RunService")
    local LocalPlayer = Players.LocalPlayer
    ...
    RunService.Heartbeat:Connect(function()
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            ...
            if not hrp or not bodyYawValue then continue end

            -- Reset on death
            if humanoid and humanoid.Health <= 0 then
                local rj = hrp:FindFirstChild("RootJoint")
                if rj and rj:GetAttribute("HSBaseC0") then
                    pcall(function() rj.C0 = rj:GetAttribute("HSBaseC0") end)
                    rj:SetAttribute("HSBaseC0", nil)
                end
                _rs[plr] = nil
                continue
            end
            ...
            -- Sample BodyYaw
            local yaw = bodyYawValue.Value
            table.insert(s.bodyYawSamples, yaw)
            if #s.bodyYawSamples > 30 then
                table.remove(s.bodyYawSamples, 1)
            end

            -- Classify AA mode
            local mode, conf = _rsClassify(s.bodyYawSamples)
            s.detectedMode = mode

            -- Calculate locked yaw
            if mode == "LEGIT" then
                s.lockedYaw = 0
                s.confidence = 1.0
            elseif mode == "STATIC" or mode == "DEFENSIVE" then
                local sum = 0
                for _, v in ipairs(s.bodyYawSamples) do sum = sum + v end
                s.lockedYaw = sum / #s.bodyYawSamples
                s.confidence = conf
            elseif mode == "JITTER" then
                local modeYaw, modeConf = _rsJitterMode(s.bodyYawSamples)
                s.lockedYaw = modeYaw
                s.confidence = modeConf
            end

            -- Visual lock: rotate RootJoint to undo the desync
            -- This makes the target appear at their REAL position
            if s.confidence > 0.3 and mode ~= "LEGIT" then
                local rj = hrp:FindFirstChild("RootJoint")
                if rj then
                    if not rj:GetAttribute("HSBaseC0") then
                        rj:SetAttribute("HSBaseC0", rj.C0)
                    end
                    pcall(function()
                        -- Negate the locked yaw to undo visual desync
                        rj.C0 = rj:GetAttribute("HSBaseC0") * CFrame.Angles(0, -math.rad(s.lockedYaw), 0)
                    end)
                end
            else
                local rj = hrp:FindFirstChild("RootJoint")
                    ...
                    if action == "Shoot" then
                        ...
                        if target and target.Character and target.Character:FindFirstChild("Head") then
                            ...
                            if HitPos and dmgpart then
                                ...
                                if isTesting then
                                    -- ===== TESTING RESOLVER v3 — BODY YAW READER =====
                                    -- Reads the game's own BodyYaw to find the REAL hitbox position.
                                    -- Uses the same formula as the game's built-in resolver.
                                    -- For JITTER: uses histogram mode + safety yaw noise.

                                    local hitPos = GetResolvedHitPos(target, HitPos)
                                    if not hitPos then
                                        -- Fallback: try current part position
                                        local part = target.Character:FindFirstChild(HitPos)
                                        hitPos = part and part.Position or (target.Character:FindFirstChild("Head") and target.Character.Head.Position)
                                    end

                                    -- Apply movement prediction (server delay + optional forward prediction)
                                    local serverDelay = getgenv().TestingResolver.ServerDelayTicks or 2
                                    local predTicks = getgenv().TestingResolver.PredictionTick or 0
                                    local netOffset = predTicks - serverDelay
                                    if netOffset ~= 0 then
                                        local predictedPos = GetPredictedPosition(target, HitPos, netOffset)
                                        if predictedPos then
                                            -- Blend between resolved position and predicted position
                                            hitPos = hitPos:Lerp(predictedPos, math.abs(netOffset) / 10)
                                        end
                                    end

                                    -- Origin: LagPeak-safe or original
                                    local origin
                                    if getgenv().LagPeakEnabled and getgenv().LagPeakSafeOrigin then
                                        origin = getgenv().LagPeakSafeOrigin
                                    elseif typeof(args[6]) == "Vector3" then
                                        ...
                                        origin = myRoot and myRoot.Position or Vector3.new(0, 0, 0)
                                    end

                                    -- Calculate and set args
                                ...
                                elseif AutoPart then
                                    ...
                                    -- Fix origin when LagPeak is active
                                    if getgenv().LagPeakEnabled and getgenv().LagPeakSafeOrigin then
                                        args[6] = getgenv().LagPeakSafeOrigin
                                    end
...
if not getgenv().LagPeakSafeOrigin then
    getgenv().LagPeakSafeOrigin = nil
end
...
task.spawn(function()
    ...
    RunService.Heartbeat:Connect(function()
        if not Root or not Root.Parent then
            ...
            getgenv().LagPeakSafeOrigin = prePeakCF.Position
            isPeaking = false
            posBuffer = {}
            return
        end
        ...
        if not getgenv().LagPeakEnabled then
            prePeakCF = currentCF
            getgenv().LagPeakSafeOrigin = currentCF.Position
            isPeaking = false
            return
        end
...
do
    ...
    ExtaSect:AddSlider({
        Name = "Jitter Safety Yaw",
        Flag = "TestingJitterSafety",
        Default = 5,
        Min = 0,
        Max = 30,
        Round = 0,
        Callback = function(v)
            getgenv().TestingResolver.JitterSafetyYaw = v
        end
    })

    ExtaSect:AddSlider({
        Name = "Server Delay Ticks",
        Flag = "TestingServerDelay",
        Default = 2,
        Min = 0,
        Max = 10,
        Round = 0,
        Callback = function(v)
            getgenv().TestingResolver.ServerDelayTicks = v
        end
    })
